...
#生成CSV文件
out_put_file = '../historical_data/BITMEX_' + symbol.replace("/","") + '_' + period.upper() + '.csv'
...
while begin_date <= end_date:
    #date_str = begin_date.strftime("%Y-%m-%dT%H:%M:%S")
...
for date in date_list:
    while True:
        try:
            ...
            print(date)
            datas = bitmex.fetch_ohlcv(symbol=symbol,timeframe=period,since=since,limit=720)
            for data in datas:
